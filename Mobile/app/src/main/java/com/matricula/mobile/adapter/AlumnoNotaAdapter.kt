...
class AlumnoNotaAdapter(val c: Context, val InscripcionList:ArrayList<Inscripcion>): RecyclerView.Adapter<AlumnoNotaAdapter.InscripcionViewHolder>(), Filterable
{
    ...
    override fun onBindViewHolder(holder: InscripcionViewHolder, position: Int) {
        ...
        holder.name.text = newList!!.estudiante.nombre
        holder.mbNum.text = newList!!.estudiante.cedula
    }
            ...
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                if (charSearch.isEmpty()) {
                    ...
                    for (row in InscripcionList) {
                        if (row.estudiante.nombre!!.toLowerCase().contains(charSearch.toLowerCase()) ||
                            row.estudiante.cedula!!.toLowerCase().contains(charSearch)) {
                            resultList.add(row)
                        }
                    }
