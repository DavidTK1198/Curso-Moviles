...
class RegistroNotaAdapter(val c: Context, val GrupoList:ArrayList<Grupo>): RecyclerView.Adapter<RegistroNotaAdapter.GrupoViewHolder>(), Filterable
{
                ...
                if (charSearch.isEmpty()) {
                    ...
                    for (row in GrupoList) {
                        if (row.curso.nombre!!.toLowerCase().contains(charSearch.toLowerCase()) ||
                            row.horario!!.toLowerCase().contains(charSearch)) {
                            resultList.add(row)
                        }
                    }
